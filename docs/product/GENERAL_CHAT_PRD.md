...
## 18. Owner Decisions (Resolved & Deferred)

**Resolved via ADR-0002:**
- Credit price: Tiered (1, 2, or 3 credits based on budget manager).
- Provider/model selection: OpenRouter (USDC) as primary MVP gateway.
- Conversation retention: 90 days.
- Canceled partial responses: Released completely; no charge for partial output.
- Admin visibility: Metadata only (count, credits, status), no conversation content.

**Deferred to D2 Technical Contracts:**
- Exact token/cost boundaries for Chat tiers 1, 2, and 3.
...
- Connection, first-token, idle-stream, and total timeout budgets (initial target: 60s).
- Conversation summarization trigger and max summary-token budget.
